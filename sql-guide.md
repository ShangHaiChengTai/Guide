...
- 
- 必须使用 `模块名_表名` 来作为数据表的完整名称，且必须使用英文全名，并且采用小写和下划线分割的规则，如果真的太长需要缩写，则采用首4个字母，尽量做到语义要明确，不能让人产生困惑。
...
- 每个表都**必须**有一个UUID主键 `id`。

- 每个表都**必须**使用laravel自动创建timestamp时间戳格式。
 
- 每个表都**必须**使用laravel自动创建软删除格式。

- 每个表的核心字段**必须**与表明类似。如:personal_rule表的核心字段为rule。
